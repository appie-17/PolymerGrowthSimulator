...
if __name__ == '__main__':
    ...
    compareDist = medianFoldNorm('Data/polymer_300k.xlsx', polymer, [1,5,5,5,5,5])

    param_boundaries = np.array([[8000, 10000], [100000, 150000], [300000000, 500000000],
    ...
    np.save(file='eval1_300k_MAT_PI_ls_1', arr=ybest)
    pickle.dump(model, open("model1_300k_MAT_PI_ls_1.p", "wb"))
    ...
    xp, yp, ybest, model = bayesianOptimisation(n_iters=150, costFunction=compareDist.costFunction,
                                                ...
                                                acquisitionFunction='probability_improvement', epsilon=1e-5)
    # np.savetxt(fname='testitest', X=yp)
    np.save(file='eval2_300k_MAT_PI_ls_1', arr=ybest)
    pickle.dump(model, open("model2_300k_MAT_PI_ls_1.p", "wb"))
    ...
    # np.savetxt(fname='testitest', X=yp)
    np.save(file='eval3_300k_MAT_EI_ls_1', arr=ybest)
    pickle.dump(model, open("model3_300k_MAT_EI_ls_1.p", "wb"))
    ...
    xp, yp, ybest, model = bayesianOptimisation(n_iters=150, costFunction=compareDist.costFunction,
                                                ...
                                                acquisitionFunction='expected_improvement', epsilon=1e-5)
    # np.savetxt(fname='testitest', X=yp)
    np.save(file='eval4_300k_MAT_EI_ls_1', arr=ybest)
    pickle.dump(model, open("model4_300k_MAT_EI_ls_1.p", "wb"))
